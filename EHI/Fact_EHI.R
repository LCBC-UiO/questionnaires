Fact_EHI = function(DATA){
  ...
  #Grab raw EHI data from the data frame for easier handling
  EHI = DATA %>% select(matches("^EHI_[0-9][0-9]$"))
  
  EHI = EHI %>% 
    
    #Caclulate the sum of positive and negative values
    mutate(POS = apply(EHI, 1:2, function(x) ifelse(x > 0, x, 1)) %>% abs %>% rowSums,
           NEG = apply(EHI, 1:2, function(x) ifelse(x < 0, x, 1)) %>% abs %>% rowSums) %>%
  ...
  #Merge the created data.frame with the incoming data
  DATA3 = cbind.data.frame(DATA %>% select(-contains("EHI")), EHI)
