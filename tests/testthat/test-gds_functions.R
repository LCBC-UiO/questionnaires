...
test_that("check gds_alter_values", {
  ...
  tmp <-
    gds_alter_values(gds, cols = gds_01, value = gds_values(1, 0))
  expect_equal(tmp$gds_01,
               rep(1, 10))
  
  tmp <-
    gds_alter_values(gds, cols = gds_05, value = gds_values(1, 0))
  expect_equal(tmp$gds_05,
               c(rep(1, 3), 0, rep(1, 5), 0))
  
  tmp <-
    gds_alter_values(gds,
                     cols = gds_05,
                     reverse = TRUE,
                     value = gds_values(1, 0))
  expect_equal(tmp$gds_05,
...
test_that("Check factor", {
  expect_equal(gds_factorise(gds_compute_sum(gds, values = gds_values(
    yes = 1, no = 0
  ))),
  structure(
    c(1L, 2L, 1L, 2L, 1L, 2L, 2L, 2L, 1L, 2L),
    .Label = c("normal",
               "mild depression",
               "severe depression"),
  ...
  expect_equal(
    as.character(gds_factorise(
      gds_compute_sum(gds, values = gds_values(yes = 1, no = 0))
    )),
    c(
      "normal",
      "mild depression",
...
test_that("Check compute", {
  expect_equal(names(gds_compute(
    gds,
